...
in the list weighted by their depth. Each element is either an integer,
...
public:
    int depthSum(const vector<NestedInteger>& nestedList) {
        ...
        int all_sum = 0;
        helper(nestedList,1,all_sum);
        return all_sum;
    }

    void helper(const vector<NestedInteger>& nestedList, int depth, int &all_sum) {
        // int sum = 0;
        for (auto ele : nestedList) {
            if (ele.isInteger()) {
                all_sum += (depth * ele.getInteger());
            } else { // nested
                helper(ele.getList(), depth+1, all_sum);
            }
        }
    }
};
    ...
    int helper(const vector<NestedInteger>& nestedList, int depth) {
        ...
        for (auto ele : nestedList) {
            if (ele.isInteger()) {
                ...
                all_sum += helper(ele.getList(), depth+1);
            }
        }
        return all_sum;
    }
